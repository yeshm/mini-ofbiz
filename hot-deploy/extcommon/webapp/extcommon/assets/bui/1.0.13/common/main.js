  ...
  function tabNav(moduleId,tabConfig,menuConfig,collapsed,homePage){
    ...
    menu.on('menuclick',function(ev){
      ...
      if(item){
        ...
        //自动关闭页签  该处暂时不能开启，如果是多个一级目录存在切换的话，会出现tab.js 1061行中获取标签到最左端距离时异常，后续在研究
        /*var closeItems = $("s.tab-item-close");
        if(closeItems.size() > 1){
            for(var i=0;i<(closeItems.size()-1);i++){
                $(closeItems.get(i)).click();
            }
        }*/
    });
    ...
    menu.on('itemselected',function(ev){
      var item = ev.item;
      if(item && item.get("isCustomerMenu") == true){
        var targetMenuModuleId = item.get("targetMenuModuleId");
        var targetMenuId = item.get("targetMenuId");
        if(targetMenuModuleId && targetMenuId){
          app.page.open({
            isClose: false,
            moduleId: targetMenuModuleId,
            id: targetMenuId
          })
        }
        return;//如果是自定义菜单，不执行默认的处理流程
      }
      if(item){
