...
(function(){
    ...
    window.onload = function(){
        //把焦点设置到文本框上
        $focus($G("videoUrl"));
        //为tab比如上传视频和插入视频tab按钮添加点击事件，做场景切换
        initTabs();
        initVideo();
        //初始化上传按钮的我们这里不需要
        //跟微信公众平台的一样，不用上传按钮就注释了  要开启也可以  --修改人 yanlch
        //initUpload();
    };
    ...
    function initVideo(){
        //创建对齐按钮,我们这里不需要
        //跟微信公众平台的一样，不用视频属性区就注释了   --修改人 yanlch
        //createAlignButton( ["videoFloat", "upload_alignment"] );
        //为input添加改变事件，并生成视频预览
    ...
    function addOkListener(){
        dialog.onok = function(){
            var currentValue = $G("videoUrl").value;
            var urlMatch= /http:\/\/[A-Za-z0-9\.-]{3,}\.[A-Za-z]{3}/;
            if(currentValue == "" || currentValue == undefined){
                alert("请输入视频地址");
                return false;
            }
            if(!urlMatch.test(currentValue)){
                alert("请输入正确地址");
                return false;
            }
            var conUrl = convert_url(currentValue);
            if(currentValue == conUrl){
                alert("只支持腾讯视频、优酷视频及土豆视频");
                return false;
            }
            $G("preview").innerHTML = "";
    ...
    function insertSingle(){
        var /*width = $G("videoWidth"),
            ...
            align = findFocus("videoFloat","name"),不需要对视频的属性进行处理就注释了    --修改人 yanlch*/
            url=$G('videoUrl').value;
        if(!url) return false;
        if ( !checkNum( [420, 280] ) ) return false;http://www.tudou.com/albumplay/Lqfme5hSolM/N2cPUnaF3ng.html
        //对腾讯视频和优酷视频的url处理 ,替换腾讯视频和优酷视频手机通用播放地址  --修改人 yanlch
        var temUrl = url.replace(/http:\/\/v\.youku\.com\/v_show\/id_([\w\-=]+)\.html/i,"http://player.youku.com/embed/$1")
            .replace(/http:\/\/v\.qq\.com\/cover\/[\w]+\/([\w\-=]+)\.html\?vid=/i,"http://v.qq.com/iframe/player.html?vid=")
            .replace(/http:\/\/www\.tudou\.com\/[\w]+\/view\/([\w\-=]+)\//i,"http://www.tudou.com/programs/view/html5embed.action?code=$1")
            .replace(/http:\/\/www\.tudou\.com\/[\w]+\/([\w\-=]+)\/([\w\-=]+)\.html/i,"http://www.tudou.com/programs/view/html5embed.action?code=$2&lcode=$1")
            .replace(/http:\/\/www\.tudou\.com\/[\w]+\/view\/([\w\-=]+)\?[\w]+/i,"http://www.tudou.com/programs/view/html5embed.action?code=$1");
        var index = -1;
        var tencent = url.match(/.qq\.com\/cover\/[\w]+\/(\w+)\.html\?vid=(\w+)/);
        var tudou = url.match(/http:\/\/www\.tudou\.com/i)
        if(tencent){
            temUrl+="&width=300&height=200&auto=0";
        }else if(tudou){
            if(temUrl.split("?").length == 3){
                index = temUrl.lastIndexOf("?");
            }
        }
        else{
            index = temUrl.lastIndexOf("?");
        ...
        if(index != -1){
            temUrl = temUrl.substring(0,index);
        }
        //
    ...
    function convert_url(s){
       return  utils.trim(s)
            ...
            .replace(/www\.tudou\.com\/programs\/view\/([\w\-]+)\/?/i, "www.tudou.com/a/$1/v.swf")
    ...
    function createPreviewVideo(url){
        ...
        /*if ( !url )return;
         ...
         //对腾讯视频的处理，暂时还没有起作用，腾讯视频的预览  修改人---yanlch
         tencent = url.match(/.qq\.com\/cover\/x\/(\w+)\.html\?vid=(\w+)/);
         if(!youkuPlay){
         if (matches){
         ...
         url = "http://v.qq.com/iframe/player.html?vid=" + tencent[2]+"";
