...
//    console.log("添加文件到队列中...")
    ...
    if (!this.fileProgressWrapper.length) {
        ...
        //插入表单
//        var formInput="<input id='mediaWallPhotoImageUrlList" + file.id + "' type='hidden'class='imgUrlHide' name='mediaWallPhotoImageUrlList'>";
...
FileProgress.prototype.setComplete = function(up, info) {
    ...
    var inputFileId=up.id;

//    console.log(res.toString());

//    var progressNameTd = this.fileProgressWrapper.find('.imgUrlHide').attr("value",res.key);


//    {"result":"success","data":{"returnBody":{"bucket":"bonn","key":"Desert.jpg","originalName":"Desert.jpg","imageSize":845941,"imageWidth":1024,"imageHeight":768,"hash":"FjBCDRqa-yvLYDNYElaa9ENaWc4X","mimeType":"image/jpeg","fileUrl":"http://bonn.qiniudn.com/Desert.jpg","fileName":"Desert.jpg"}}}
    ...
    if (res.url) {
        ...
        url = res.fileUrl;
        str = "<div><strong>Link:</strong><a href=" + res.fileUrl + " target='_blank' > " + res.url + "</a></div>" +
            "<div class=hash><strong>Hash:</strong>" + res.hash + "</div>";
    } else {
        var fileUrl,fileName;

        if("Y"==app.config.qiniuIsOpen){
            fileUrl = up.getOption('domain') + encodeURI(res.key);
            fileName=encodeURI(res.key);
        }else{
            var data=res.data;
...
//        var link = domain + res.key;
        str = "<div><strong>Link:</strong><a href=" + fileUrl + " target='_blank' > " + fileUrl + "</a></div>" +
            "<div class=hash><strong>Hash:</strong>" + res.hash + "</div>";
    }
...
//    console.log("图片url"+url);
    ...
    var showImg = $('<img src="/ext/assets/qiniuuploader/img/loading.gif"/>');

    progressNameTd.append(Wrapper);

    if (!isImg) {
        showImg.attr('src', '/ext/assets/qiniuuploader/img/default.png');
